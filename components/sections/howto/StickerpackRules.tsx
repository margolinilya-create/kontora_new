        ...
        <ul className="mt-10 space-y-3">
          {howto.stickerpackRules.bullets?.map((bullet, i) => (
            <RevealOnScroll key={bullet.id} delay={i * 0.04}>
              <li className="flex items-start gap-4 rounded-lg  bg-bg-base p-4 shadow-soft-sm">
                <span className="font-mono text-xs font-bold uppercase tracking-widest text-yellow">
                  {String(i + 1).padStart(2, '0')}
                </span>
                <p className="text-pretty text-sm leading-relaxed text-cream-soft md:text-base">
                  {bullet.text}
                </p>
              </li>
            </RevealOnScroll>
