  ...
  return (
    <>
      ...
      <section className="bg-bg-surface py-16 md:py-20">
        <Container size="md">
          <p className="mb-12 text-pretty text-lg leading-relaxed text-cream-soft">{doc.intro}</p>
          ...
          <div className="space-y-12">
            {doc.sections.map((section, i) => (
              <RevealOnScroll key={section.id} delay={i * 0.04}>
                <article
                  id={section.id}
                  className="rounded-xl border-2 border-line bg-bg-base p-6 md:p-8"
                >
                  <h2 className="font-display text-xl font-bold uppercase leading-tight md:text-2xl">
                    {section.title}
                  </h2>
                  <div className="mt-4 space-y-3 text-pretty text-base leading-relaxed text-cream-soft">
